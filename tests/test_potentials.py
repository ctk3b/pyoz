...
def test_custom_function(sig, eps):
    ...
    U_cont = oz.ContinuousPotential(potential_func=lj_func,
    ...
    U_cont.apply(r=unary_lj.r)

    # Subclassed ContinuousPotential.
    U_sub = oz.LennardJones(sig='arithmetic', eps='geometric')
...
def test_mixing_rules():
    c0 = oz.Component('0')
    c1 = oz.Component('1')
    pot = oz.LennardJones(sig='arithmetic', eps='geometric')
...
def test_partial_mixing_rules():
    c0 = oz.Component('0')
    c1 = oz.Component('1')
    pot = oz.LennardJones(sig='arithmetic')
...
def test_invalid_mixing_rules():
    with pytest.raises(PyozError):
        pot = oz.LennardJones(sig='arithmetic', epz='geometric')
    with pytest.raises(PyozError):
        pot = oz.LennardJones(sig='arithmetic',
                              eps='geometric',
                              foo='arithmetic')
