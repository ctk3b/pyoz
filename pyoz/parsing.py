...
from numpy import array, zeros, pi, int8, int32, float_
...
def parse_input(cmdline):
    ...
    if (start_name['ctrl'] == None):
        ...
        ctrl = parse_ctrl(cmdline, lines[start_index:end_index],
                          lines_orig[start_index:end_index], start_index + 1,
                          end_index + 1)
    print("")
...
def parse_ctrl(cmdline, lines, lines_orig, lineno_beg, lineno_end):
    ...
    ctrl_def = settings.ctrl_defaults()
    ctrl = {}
    ...
    try:
        for i in range(len(lines)):
            ...
            if (match(r'#', l) or (l == "")):
                ...
                if (npoints <= 0):
                    raise badValue((i,
                                    "cannot use zero/negative number of discretization points!"))
                ...
                if ((mix_param <= 0) or (mix_param > 1)):
                    raise badValue((i,
                                    "mixing parameter not in allowed range 0 < mix_param ≤ 1"))
                ...
                if (max_iter <= 0):
                    raise badValue((i,
                                    "minimum number of 1 iterations has to be performed"))
                ...
                if ((nr_mix_param < 0) or (nr_mix_param > 1)):
                    raise badValue((i,
                                    "NR mixing parameter not in allowed range 0 ≤ nr_mix_param ≤ 1"))
            elif (match(r'nr_conv_crit$', keyword)):
                nr_convergence_factor = float((l.split())[1])
                if (nr_convergence_factor <= 0.0):
                    raise badValue((i,
                                    "NR relative convergence criterion has to be larger than 0"))
            elif (match(r'nr_max_iter$', keyword)):
                nr_max_iter = int((l.split())[1])
                if (nr_max_iter <= 0):
                    raise badValue((i,
                                    "minimum number of 1 NR iterations has to be performed"))
    ...
    if (((npoints + 1) & npoints) != 0):
        ...
        print(
            "\t!!!!! you should use number of points which is a power of 2 !!!!!")
    ...
    if (ctrl['do_nr']):
        ...
        print("\tNR relative convergence factor\t%e" % ctrl[
            'nr_convergence_factor'])
...
def parse_syst(ctrl, lines, lines_orig, lineno_beg, lineno_end):
    ...
    syst = {}
    syst_def = settings.syst_defaults()
    ...
    const = settings.const(syst['temp'], syst['epsilon_r'])
    const.print_const()
    ...
    # end try
        ...
        print(
            "error processing concentration of component %u - not enough values in the input" % (
                i + 1))
        print(msg)
        sys.exit(2)
...
def parse_parm(ctrl, syst, const, lines, lines_orig, lineno_beg, lineno_end):
    ...
    parm = []
    parm_def = settings.parm_defaults()
    ...
    for pot_index in range(len(start_pos)):
        ...
        if (potential == 'hs'):
            ...
            (unit_name, unit_factor) = _check_distance_unit(const, hs_unit,
                                                            hs_unit_orig)
            print(unit_name)
            ...
            dis_index = zeros((syst['ncomponents'], syst['ncomponents']),
                              dtype=int32)
            ...
            for i in range(syst['ncomponents']):
                for j in range(i, syst['ncomponents']):
                    ...
                    if (hs_ij[i, j] > ctrl['max_r']):
                        print(
                            "hs contact distance (%f) beyond the discretization interval (max=%f)" % (
                    ...
                    if (hs_ij[i, j] != ctrl['deltar'] * (dis_index[i, j] + 1)):
                        print(
                            "\tWARNING: hard sphere diameter (combination %u,%u)\n\t\t\t%f not an integer multiple of step size\n\t\t\t%f used instead!" % (
                                (i + 1), (j + 1), hs_ij[i, j],
                                ctrl['deltar'] * (dis_index[i, j] + 1)))
                        print(
        ...
        # ****************************************
            ...
            if (syst['closure_name'] == 'py'):
                print(
                    "\tuse of long-ranged Coulomb potential is not consistent")
            ...
            # end if(erf_alpha_loaded), else follows
                ...
                kappa = sqrt(
                    8.0 * pi * const.bjerrum_length * const.Na * ionic_strength * 1e-4)
            ...
            if (chg_inddip):
                ...
                print(
                    "\tpotential type\t\t\t1/r^4 charge-induced dipole (chg_inddip)")
                ...
                for i in range(syst['ncomponents']):
                    for j in range(syst['ncomponents']):
                        chgdip_ij[i, j] = -0.5 * const.bjerrum_length * charge[
                                                                            i] ** 2 * \
                                          dip_alpha[j]
                        # end for j in range(ncomponents)
                        # end for i in range(ncomponents)
        ...
        # ****************************************
            ...
            if (sigma_loaded):
                if (sigma.any() < 0.0):
                    print(
                        "invalid entry for sigma! all values must be larger than 0.0")
                    sys.exit(2)
            else:
            ...
            print("\tsigma units\t\t\t"),
            # if there is some problem, it will be treated in the function!
            (unit_name, unit_factor) = _check_distance_unit(const, sigma_unit,
                                                            sigma_unit_orig)
            print(unit_name)
            sigma *= unit_factor
            ...
            print("\tepsilon units\t\t\t"),
            # if there is some problem, it will be treated in the function!
            (unit_name, unit_factor) = _check_energy_unit(const, epsilon_unit,
                                                          epsilon_unit_orig)
            print(unit_name)
            epsilon *= unit_factor
        ...
        # ****************************************
            ...
            for i in range(1, len(pot_data) - 1):
                ...
                # end try
                    ...
                    print(
                        "error converting numerical input data on line %lu (%s)" % (
                            i + lineno_pot, l))
                    print(msg)
                    sys.exit(2)
                except unkKeyword as msg:
            ...
            print("\tpmf units\t\t\t"),
            # if there is some problem, it will be treated in the function!
            (unit_name, unit_factor) = _check_energy_unit(const, pmf_unit,
                                                          pmf_unit_orig)
            print(unit_name)
            ...
            # if there is some problem, it will be treated in the function!
            (unit_name, unit_factor) = _check_distance_unit(const,
                                                            pmf_distance_unit,
                                                            pmf_distance_unit_orig)
            print(unit_name)
            ...
            if (pmf_interp_type == 'linear'):
                print("linear")
                from pyoz.misc import interpolate_linear as interpolate
            elif (pmf_interp_type == 'cosine'):
                print("cosine")
                from pyoz.misc import interpolate_cosine as interpolate
            else:
                # unknown unit
            ...
            if (pmf_single == False):
                if (len(pmf_filenames) != num_comb):
                    print(
                        "not enough file names with individual pmf. needed %u got %u" % (
            ...
            try:
                ...
                if (pmf_single == True):
                    ...
                    if (len(datasets) < 2):
                        ...
                        raise pmfError(("need at least 2 points, got %u " % len(
                            datasets[0])))
                    fr.close()
                    # pre-allocate arrays
                    # distance
                    ...
                    for i in range(num_comb):
                        ...
                        if (len(datasets[i]) < 2):
                            # we need at least 2 points
                            raise pmfError((
                                "need at least 2 points, got %u " % len(
                                    datasets[i])))
                        fr.close()
                ...
                for i in range(num_comb):
                    ...
                    print(
                        "\t\tcombination %u of %u (file %s, columns 1 and %u)" % (
                    ...
                    for line in c_dataset:
                        ...
                        if (index == 0):
                            ...
                            if ((pmf_r[i][index] <= pmf_rmin[i]) or (
                                        pmf_r[i][index] <= pmf_rmax[i])):
                                raise pmfError((
...
def parse_outp(cmdline, lines, lines_orig, lineno_beg, lineno_end):
    ...
    outp_def = settings.outp_defaults()
    outp = {}
    ...
    if (outp['G_ij_write']):
        if (outp['G_ij_savefreq'] == 0):
            ...
            print(
                "\tGamma function will be saved every %lu iterations to %s" % (
...
def check_validity(lines, lineno_beg, start_pos, end_pos):
    ...
    if (len(end_pos) != len(start_pos)):
        print(
            "\tnumber of section openings (%u) doesn't agree with the number of %%end statements (%u)" % (
    ...
    for i in range(start_pos[0] - 1):
        ...
        if (not (match("#", lines[i]) or lines[i] == "")):
            print(
                "\tunknown identifier (%s) outside of declaration block on line %u" % (
