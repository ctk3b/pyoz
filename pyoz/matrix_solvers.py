...
def solver_2(a, b, np):
    ...
    if (a_det == 0.0).any():
        print(
            "singular matrix, cannot invert: determinant of (1-C) matrix is zero for dr=%u" % dr)
        exit(1)
...
def solver_3(a, b, np):
    ...
    for dr in range(np):
        ...
        a_inv[1, 0] *= -1.0 * (
        a[1, 0, dr] * a[2, 2, dr] - a[2, 0, dr] * a[1, 2, dr])
        a_inv[2, 0] *= (a[1, 0, dr] * a[2, 1, dr] - a[2, 0, dr] * a[1, 1, dr])
        a_inv[0, 1] *= -1.0 * (
        a[0, 1, dr] * a[2, 2, dr] - a[2, 1, dr] * a[0, 2, dr])
        a_inv[1, 1] *= (a[0, 0, dr] * a[2, 2, dr] - a[2, 0, dr] * a[0, 2, dr])
        a_inv[2, 1] *= -1.0 * (
        a[0, 0, dr] * a[2, 1, dr] - a[2, 0, dr] * a[0, 1, dr])
        a_inv[0, 2] *= (a[0, 1, dr] * a[1, 2, dr] - a[1, 1, dr] * a[0, 2, dr])
        a_inv[1, 2] *= -1.0 * (
        a[0, 0, dr] * a[1, 2, dr] - a[1, 0, dr] * a[0, 2, dr])
