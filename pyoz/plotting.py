...
# python modules
import pylab
from numpy import isnan, isinf, copy, zeros
...
def plot_initialize(ctrl, syst, const, r):
    ...
    for i in range(syst['ncomponents']):
        for j in range(i, syst['ncomponents']):
            p_U_ij.append(pylab.plot(r, zeros(ctrl['npoints']))[0])
            p_U_ij[-1].set_label(
                "Uerf(%s,%s)" % (syst['name'][i], syst['name'][j]))
    pylab.legend()
    ...
    for i in range(syst['ncomponents']):
        for j in range(i, syst['ncomponents']):
            ...
            p_G_ij[-1].set_label(
                "Gams(%s,%s)" % (syst['name'][i], syst['name'][j]))
    pylab.legend()
    ...
    for i in range(syst['ncomponents']):
        for j in range(i, syst['ncomponents']):
            ...
            p_c_ij[-1].set_label(
                "cs(%s,%s)" % (syst['name'][i], syst['name'][j]))
    pylab.legend()
    for i in range(syst['ncomponents']):
        for j in range(i, syst['ncomponents']):
            p_c_ij.append(pylab.plot(r, zeros(ctrl['npoints']))[0])
            p_c_ij[-1].set_label(
                "cf(%s,%s)" % (syst['name'][i], syst['name'][j]))
    pylab.legend()
...
def plot_update(syst, const, U_r=None, U_erf=None, G_r=None, c_r=None, g_r=None,
                c_f=None):
